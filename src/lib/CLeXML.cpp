...
namespace XML {
  ...
  // Methods 
  void CLeXML::setArquivo(const std::string& A){pArquivo = A;pLeGera();}
  std::string CLeXML::getArquivo(void) const {return pArquivo;}

  void  CLeXML::setElemento(const std::string& B){pElemento = B;pLeGera();}
  ...
  // Member function - XML
  const inline ptree& CLeXML::limpa_ptree(void) const {static ptree t;return t;}
  inline void CLeXML::pZera(void){pLin=0;pElem=0;pCol=0;}

  void CLeXML::pLeGera(void){ // Le os dados do XML e gera a matriz
    try {
      pZera();
      ...
      BOOST_FOREACH(const ptree::value_type& f, Elemento){
        ...
        const ptree& Atributos = f.second.get_child("<xmlattr>", limpa_ptree()); // limpa o ptree
        BOOST_FOREACH(const ptree::value_type& v, Atributos){
          if(v.first != "<xmlcomment>" && !v.second.data().empty()) //se nao for comentario ou nao estiver vazio
            ...
            std::cerr << "\n[LeXML] Nao ha atributos na tag \"" << v.first << "\" do ID " << pLin << "\ndo elemento raiz " << this->getElemento() << " do arquivo " << this->getArquivo() << "\n\n";
            exit(100);
          }
        }
      }
