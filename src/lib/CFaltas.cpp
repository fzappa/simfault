...
void CFault::setZbus(const Matriz &MA) { pMZ = MA; }

void CFault::setVbus(const Vetor &VB) { pVE = VB; }

void CFault::setBusFault(const int &bus) { pBus = bus; }
...
Matriz CFault::getLLL(void)
{
    ...
    Iabc.imag().array() =
        ((pCompSim * I012).array().imag() / (pCompSim * I012).array().real()).atan() * radiano2grau;
    ...
    for (uint i = 0; i < Vbarra.size(); ++i) {
        // Tensao na barra
        Saida(i, 1) = Tensoes(i).imag() -
                      (pMZ(i, pBus - 1).imag() * (-pIFault.imag()));  // Ignorando as resistencias
