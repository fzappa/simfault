...
const Matriz KronRN(const Matriz &A, int N)
{
    ...
    if (N <= 0)
        ...
        Matriz temp{A};
        for (uint i = uint(temp.rows()) - 1; i >= uint(N); --i) temp = Kron(temp);
    ...
    if ((L - 1) < 0)  // Remover a barra 1 = eliminar a linha e coluna 0
        ...
        for (uint i = 0; i < uint(A.rows()); ++i) {
            for (uint j = 0; j < uint(A.rows()); ++j) {
                k++;
                if ((i != uint(L - 1)) &
                    (j != uint(L - 1))) {  // Se linha ou coluna forem diferentes da removida
                    // Mapeia os dados para um vetor sequencial
                    Dados.push_back(A(i, j) -
                                    (A(i, (L - 1)) * A((L - 1), j)) / A((L - 1), (L - 1)));
                }
            }
        }
        ...
        for (uint i = 0; i < uint(temp.rows()); ++i) {
            for (uint j = 0; j < uint(temp.rows()); ++j) {
                if (Dados[k] != tipo(0)) {
                    temp(i, j) =
                        Dados[k];  // Varre o vetor e atribui os elementos diferentes de zero
                    ++k;
                }
            }
        }
